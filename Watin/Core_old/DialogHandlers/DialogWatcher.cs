	...
	public class DialogWatcher : IDisposable
	{
				...
				lock (this)
				{
					foreach (IDialogHandler dialogHandler in handlers)
					{
						...
						catch (Exception e)
						{
							...
							Logger.Log.Logger.LogData("Exception was thrown while DialogWatcher called HandleDialog:", Logger.LogLevel.Debug);
                            Logger.Log.Logger.LogData(e.ToString(), Logger.LogLevel.Debug);
                        }
					}
					...
					{
						Logger.Log.Logger.LogData("Auto closing dialog with title '{0}'."+ window.Title, Logger.LogLevel.Debug);
                        window.ForceClose();
					}
				}
			}
		...
		private static void WaitUntilVisibleOrTimeOut(Window window)
		{
			...
			do
			{
                ...
                if(!SelectHelper.DialogWacher) return;

                Thread.Sleep(50);
			} while (!timer.Elapsed);

			Logger.Log.Logger.LogData("Dialog with title '{0}' not visible after {1} seconds."+ window.Title, Logger.LogLevel.Debug);
        }
