    ...
    public sealed class AutomationElementTests : AutomationElementWrapper
    {
        ...
        [TestCaseAttribute("HwndWindowRect.2.UIA",
           TestSummary = "Verify UIA rectangle available",
            ...
            Description = new string[] 
                {
                "Verification: Obtain AutomationElement.GetBoundingRectangle (regression for TitleBar / WindowsOSBug:1559184",
                })]
        ...
        public void TestKeyboardFocusable111(TestCaseAttribute testCase)
        {
            ...
            // "Precondition: IsKeyboardFocusable == false",
            TSC_VerifyPropertyEqual(pattern_GetProperty(m_le, AutomationElement.IsKeyboardFocusableProperty), false, AutomationElement.IsKeyboardFocusableProperty, CheckType.IncorrectElementConfiguration);

            // "Verify: call SetFocus and expect an exception to be thrown",
            TS_SetFocus(m_le, typeof(InvalidOperationException)/*null*/, CheckType.Verification);

            // "Verify: IsKeyboardFocusable still false"
            TSC_VerifyPropertyEqual(pattern_GetProperty(m_le, AutomationElement.IsKeyboardFocusableProperty), false, AutomationElement.IsKeyboardFocusableProperty, CheckType.Verification);
        }
        ...
        public void TestKeyboardFocusable112(TestCaseAttribute testCaseAttribute)
        {
            ...
            // "Precondition: IsKeyboardFocusable == true",
            TSC_VerifyPropertyEqual(pattern_GetProperty(m_le, AutomationElement.IsKeyboardFocusableProperty), true, AutomationElement.IsKeyboardFocusableProperty, CheckType.IncorrectElementConfiguration);

            // "Verify: call SetFocus set focus to the element",
            TS_SetFocus(m_le, null, CheckType.Verification);

            //"Verify: IsKeyboardFocusable still true"
            TSC_VerifyPropertyEqual(pattern_GetProperty(m_le, AutomationElement.IsKeyboardFocusableProperty), true, AutomationElement.IsKeyboardFocusableProperty, CheckType.Verification);
        }
        ...
        public void TestIsPasswordProperty132(TestCaseAttribute testCaseAttribute)
        {
            ...
            // "Precondition: AutomationElement.IsPasswordProperty == true",
            TSC_VerifyPropertyEqual(pattern_GetProperty(m_le, AutomationElement.IsPasswordProperty), true, AutomationElement.IsKeyboardFocusableProperty, CheckType.IncorrectElementConfiguration);
        ...
        public void HelpTextProperty152(TestCaseAttribute testCaseAttribute)
        {
            ...
            // "Verification: HelpTextProeprty != null",
            TSC_VerifyProperty(m_le.Current.HelpText, null, false, AutomationElement.HelpTextProperty, CheckType.IncorrectElementConfiguration);

            // "Verification: HelpTextProeprty != \"\"",
            TSC_VerifyProperty(m_le.Current.HelpText, "", false, AutomationElement.HelpTextProperty, CheckType.IncorrectElementConfiguration);

            // "Verification: HelpTextProeprty != AutomationElement.NotSupported",
            TSC_VerifyProperty(m_le.Current.HelpText, AutomationElement.NotSupported, false, AutomationElement.HelpTextProperty, CheckType.IncorrectElementConfiguration);
        }
        ...
        [TestCaseAttribute("AutomationIdProperty.1.7.8",
            ...
            Description = new string[] 
            {
                ...
                "Precondition: ControlType != TitleBar (known item that does not support AutomationIdProperty)",
        ...
        public void AutomationIdProperty178(TestCaseAttribute testCaseAttribute)
        {
            ...
            TSC_VerifyProperty(ct, null, false, AutomationElement.ControlTypeProperty, CheckType.IncorrectElementConfiguration);
            TSC_VerifyPropertyEqual((bool)m_le.GetCurrentPropertyValue(AutomationElement.IsWindowPatternAvailableProperty), false, AutomationElement.IsWindowPatternAvailableProperty, CheckType.IncorrectElementConfiguration);
            ...
            TSC_VerifyProperty(Helpers.GetProgrammaticName(ct), Helpers.GetProgrammaticName(ControlType.ListItem), false, AutomationElement.ControlTypeProperty, CheckType.IncorrectElementConfiguration);
            TSC_VerifyProperty(Helpers.GetProgrammaticName(ct), Helpers.GetProgrammaticName(ControlType.DataItem), false, AutomationElement.ControlTypeProperty, CheckType.IncorrectElementConfiguration);
            TSC_VerifyProperty(Helpers.GetProgrammaticName(ct), Helpers.GetProgrammaticName(ControlType.TreeItem), false, AutomationElement.ControlTypeProperty, CheckType.IncorrectElementConfiguration);
            TSC_VerifyProperty(Helpers.GetProgrammaticName(ct), Helpers.GetProgrammaticName(ControlType.TabItem), false, AutomationElement.ControlTypeProperty, CheckType.IncorrectElementConfiguration);
            TSC_VerifyProperty(Helpers.GetProgrammaticName(ct), Helpers.GetProgrammaticName(ControlType.TitleBar), false, AutomationElement.ControlTypeProperty, CheckType.IncorrectElementConfiguration);
            if (testParentType)
            {
                ControlType pct = TreeWalker.ControlViewWalker.GetParent(m_le).Current.ControlType;
                TSC_VerifyProperty(Helpers.GetProgrammaticName(pct), Helpers.GetProgrammaticName(ControlType.ListItem), false, AutomationElement.ControlTypeProperty, CheckType.IncorrectElementConfiguration);
                TSC_VerifyProperty(Helpers.GetProgrammaticName(pct), Helpers.GetProgrammaticName(ControlType.DataItem), false, AutomationElement.ControlTypeProperty, CheckType.IncorrectElementConfiguration);
            }
            else
            {
                m_TestStep += 2;
        ...
        public void RuntimeId(TestCaseAttribute testCaseAttribute)
        {
            HeaderComment(testCaseAttribute);
            ...
            TSC_VerifyProperty(f.GetType(), typeof(int[]), true, "Type", CheckType.Verification);
        ...
        public void TestBoundingRect2(TestCaseAttribute testCaseAttribute)
        {
            ...
            {
                // See 1205414 Collapsing zero-area Rects to Rect.Empty breaks sparkle UIA testing
        ...
        public void TestSetFocus1a(TestCaseAttribute testCaseAttribute)
        {
            ...
            //"Step: Verify that the AutomationElement's HasKeyboardFocusProperty == false",
            TSC_VerifyPropertyEqual(m_le.Current.HasKeyboardFocus, false, AutomationElement.HasKeyboardFocusProperty, CheckType.IncorrectElementConfiguration);
        ...
        public void TestSetFocus4(TestCaseAttribute testCase)
        {
            ...
            // "If IsKeyboardFocusableProperty == true, SetFocus should not throw an exception, else it should throw InvalidOperationException"            
            // In Native UIA, SetFocus is not throwing error if IsKeyboardFocusableProperty is false
            // Win7 BUG: 745103 and Dev WI #120922 which will be fixed in M3
            #if NATIVE_UIA            
            exType = null;
            #endif
        ...
        public void TestSetFocus10(TestCaseAttribute testCaseAttribute)
        {
            ...
            //"Precondition: ControlType is ControlType.ListBox",
            TSC_VerifyPropertyEqual(m_le.Current.ControlType, ControlType.List, AutomationElement.ControlTypeProperty, CheckType.IncorrectElementConfiguration);

            //"Precondition: Verify that the element's IsOffScreen = false",
            TSC_VerifyPropertyEqual(m_le.Current.IsOffscreen, false, AutomationElement.IsOffscreenProperty, CheckType.IncorrectElementConfiguration);

            //"Precondition: IsSelectionRequired = false",
            TSC_VerifyPropertyEqual((bool)m_le.GetCurrentPropertyValue(SelectionPattern.IsSelectionRequiredProperty), false, SelectionPattern.IsSelectionRequiredProperty, CheckType.IncorrectElementConfiguration);
            ...
            //"Step: Verify that the AutomationElement's HasKeyboardFocusProperty == false",
            TSC_VerifyPropertyEqual(m_le.Current.HasKeyboardFocus, false, AutomationElement.HasKeyboardFocusProperty, CheckType.Verification);
        ...
        public void TestClickablePointProperty2(TestCaseAttribute testCaseAttribute)
        {
            ...
            {
                ThrowMe(CheckType.KnownProductIssue, "This is bug 1974400");
            }
            m_TestStep++;
        }
