...
namespace Microsoft.Test.UIAutomation.Tests.Patterns
{
    ...
    public sealed class ScrollTests : ScrollPatternWrapper
    {
        ...
        [TestCaseAttribute("SetScrollPercent.S.1.1",
            ...
            Status = TestStatus.Problem, // Incorrect steps
        ...
        public void TestSetScrollPercent16(TestCaseAttribute testCaseAttribue)
        {
            ...
            /* TODO: Bug workaround, event shsould be EventFired.False */
        ...
        public void TestSetScrollPercent17(TestCaseAttribute testCaseAttribue)
        {
            HeaderComment(testCaseAttribue);
...
#if NATIVE_UIA
            this.TestC_SetScrollPercent(HScrollable, VScrollable, _NOSCROLL, _ZERO, _NOSCROLL, 101D, _NOSCROLL, _ZERO, EventFired.False, EventFired.False, typeof(ArgumentException));
#else
...
#if NATIVE_UIA
            this.TestC_SetScrollPercent(HScrollable, VScrollable, _ZERO, _NOSCROLL, newH, _NOSCROLL, _ZERO, _NOSCROLL, EventFired.Undetermined, EventFired.Undetermined, typeof(ArgumentException));
#else
        ...
        [TestCaseAttribute("SetScrollPercent.S.1.39",
            Priority = TestPriorities.Pri1,
            Status = TestStatus.Problem, // Description does not match to the test
            Author = "Microsoft Corp.",
            TestCaseType = TestCaseType.Events, EventTested = "AutomationPropertyChangedEventHandler(ScrollPattern.HorizontalScrollPercentProperty)/AutomationPropertyChangedEventHandler(ScrollPattern.VerticalScrollPercentProperty)",
            Description = new string[] 
        {
        ...
        [TestCaseAttribute("Scroll.S.2.18",
            Priority = TestPriorities.Pri1, /* Pri0 */
            Status = TestStatus.Works,
            Author = "Microsoft Corp.",
            TestCaseType = TestCaseType.Events, EventTested = "AutomationPropertyChangedEventHandler(ScrollPattern.HorizontalScrollPercentProperty)/AutomationPropertyChangedEventHandler(ScrollPattern.VerticalScrollPercentProperty)",
            Description = new string[] {
        ...
        public void Scrollable1(TestCaseAttribute testCaseAttribue)
        {
            ...
            //"Verify: If HorizontalScrollable = false, then HorizontalScrollPercent <= 0", 
            // TODO
            if (pattern_getHorizontallyScrollable.Equals(false))
                if (pattern_getHorizontalScrollPercent > _ZERO) 
                    ThrowMe(CheckType.Verification, "HorizontalScrollable == false, but HorizontalScrollPercent != 0");
            ...
            if (pattern_getVerticallyScrollable.Equals(false))
                if (pattern_getVerticalScrollPercent > _ZERO)
                    ThrowMe(CheckType.Verification, "VerticallyScrollable == false, but VerticalScrollPercent != 0");
            m_TestStep++;
        ...
        void TSC_VerifyScrollPercent(object ExpectedScrollPos, ScrollDirection scrollDirection, CheckType ct)
        {
            ...
            {
                if ((double)ExpectedScrollPos == _NOSCROLL)
                {
                    ExpectedScrollPos = _ZERO;
                }
                ...
                if ( ((double)val != _NEGONE) && (Math.Abs((double)val - (double)ExpectedScrollPos) > ACCURACY) )
