...
namespace InternalHelper.Tests.Patterns
{
    ...
    public class TextTestsHelper : TextWrapper
    {
        ...
        {
            // Bug1134041: TextPattern.TextSelectionChangedEvent does not fire unless control has focus.
            m_le.SetFocus();
        ...
        internal void TS_Move(TextPatternRange callingRange, TextUnit[] supportedTextUnits, int[] numberOfTextUnits, Count countEnum, Count expectedCountEnum, CheckType checkType)
        {
            ...
            for (unit = TextUnit.Character; unit <= TextUnit.Document; unit++)
            {
                ...
                if (supportedTextUnits[(int)unit] == unit)
                {
                    ...
                    {
                        errorCount++;
                        Comment(KNOWNISSUE + " Actual move count    = " + actualCount + " WHICH DOES NOT EQUAL EXPECTED COUNT");
                    }
                }
                else
                {
        ...
        internal void TS_MoveEndpointByUnitAndValidate(TextPatternRange callingRange, TextPatternRangeEndpoint endPoint, int count, int expectedCount, int richEditOffset, CheckType checkType)
        {
            ...
            for (unit = TextUnit.Character; unit <= TextUnit.Document; unit++)
            {
                ...
                {
                    Comment(KNOWNISSUE + " MoveEndpointByUnit(" + endPoint + "," + unit + "," + count + ") moved the range by " + actualCount + ", expected " + expectedCount);
                    errorCount++;
                }
            }
            if (errorCount > 0)
       ...
       /// -------------------------------------------------------------------
        ...
        internal static TextPattern GetDifferentTextPattern(AutomationElement element, CheckType checkType)
        {
            ...
            {
                ThrowMe(CheckType.IncorrectElementConfiguration, "There is only one AutomationElement that supports TextPattern in this app. Unable to proceed with test");
                return null;
            }
        ...
        //---------------------------------------------------------------------------
        // Valdiate if exception is a legitimate UIA Verify exception raised by a
        // ThrowMe(...) statement
        //---------------------------------------------------------------------------
        ...
        { 
            if (ex is IncorrectElementConfigurationForTestException)    // Don't count legitimate exceptions
                return true;                                            // used by UIA Verify as regression/test failures
        ...
        internal void SetValue(string actualText, CheckType checkType)
        {
            ...
            Input.SetValue(m_le, actualText);
        ...
        // Regress Bug
        //-------------------------------------------------------------------
        internal void Bug1192048( string bugID, string controls, string bugTitle )
        {
            int  count         = -1;
            int  actualCount   = 0;
            TextPatternRange range       = null;
        ...
        // Regress Bug
        //-------------------------------------------------------------------
        internal void Bug1191485( string bugID, string controls, string bugTitle )
        {
            TextPatternRange range = null;
        ...
        // Regress Bug 1186712
        //-------------------------------------------------------------------
        internal void Bug1186712( string bugID, string controls, string bugTitle )
        {
            int    count       = 1;
            int    actualCount = 0;
            TextPatternRange range       = null;
        ...
        //-------------------------------------------------------------------
        // Regress Bug 1186687: HOT: MoveEndpointByUnit(End, TextUnit.Leine, 1 ) returning 1, not 0, for non-empty text range on Win32 Edit
        //-------------------------------------------------------------------
        internal void Bug1186687( string bugID, string controls, string bugTitle )
        {
            int count          = 1;
            int actualCount    = 0;
            TextPatternRange range       = null;
        ...
        //-------------------------------------------------------------------
        // Regress Bug 1186653: HOT: FindText returns range that is trimmed by one character on Win32 Edit Control
        //-------------------------------------------------------------------
        internal void Bug1186653( string bugID, string controls, string bugTitle )
        {
            string textToFind  = "";
            string textFound   = "";
            TextPatternRange range       = null;
        ...
        internal void Bug1184263( string bugID, string controls, string bugTitle )
        {
            TextPatternRange range  = null;
        ...
        //-------------------------------------------------------------------
        // Regress Bug 1103948: HOT Bug: GetBoundingRectangles() raises exception for Edit controls with Multiline style set, but no scroll bar
        //-------------------------------------------------------------------
        internal void Bug1103948( string bugID, string controls, string bugTitle )
        {
            Rect[] boundRects = new Rect[0];
        ...
        //-------------------------------------------------------------------
        // Regress Bug 1081944: BVT Blocker: GetVisibleRanges() raises assertion on edit control when size of text is 1 character
        //-------------------------------------------------------------------
        internal void Bug1081944( string bugID, string controls, string bugTitle )
        {
            TextPatternRange   range         = null;
            TextPatternRange[] visibleRanges = null;
        ...
        //-------------------------------------------------------------------
        // Regress Bug 1080760: ExpandToEnclosingUnit(TextUnit.Line) incorrectly returns an empty range on single-line Win32:Edit
        //-------------------------------------------------------------------
        internal void Bug1080760( string bugID, string controls, string bugTitle )
        {
            int  actualCount   = 0;
            string text        = "";
            TextPatternRange range       = null;
        ...
        //-------------------------------------------------------------------
        // Regress Bug 1080662: MoveEndpointByUnit(TextUnit.Line, MinInt) and Move(TextUnit.Line, MinInt) both return incorrect line count Win32:Edit/RichEdit
        //-------------------------------------------------------------------
        internal void Bug1080662( string bugID, string controls, string bugTitle )
        {
            int  actualCount1   = 0;
            int  actualCount2   = 0;
            TextPatternRange range        = null;
        ...
        //-------------------------------------------------------------------
        // Regress Bug 1080418: MoveEndpointByUnit(TextUnit.Line, MaxInt) and Move(TextUnit.Line, MaxInt) both return 0 on Win32:Edit when moving forward
        //-------------------------------------------------------------------
        internal void Bug1080418( string bugID, string controls, string bugTitle )
        {
            int actualCount1 = 0;
            int actualCount2 = 0;
            TextPatternRange range = null;
        ...
        //-------------------------------------------------------------------
        // Regress Bug 1055455: BVT Blocker: TextPatternRange.GetVisibleRanges returns text longer than actual visible range on Edit w/Horiz Scrollbar
        //-------------------------------------------------------------------
        internal void Bug1055455( string bugID, string controls, string bugTitle )
        {
            string actualText  = "";
            TextPatternRange range        = null;
            TextPatternRange[] visibleRanges = null;
            ...
            try
            {
                ...
                // Wild approximation. Bug repro had typo and didn't give exact values to expect previously
                // We don't have an independent way to easily verify what text is currently visible without
                // doing more win32 programming than seems worth the effort
        ...
        // Regress Bug
        //-------------------------------------------------------------------
        internal void Bug1139305( string bugID, string controls, string bugTitle )
        {
            string      text = "";
        ...
        //-------------------------------------------------------------------
        // Regress Bug 1113251: BVT BLOCKER: TextPatternRange.ScrollIntoView incorrectly scrolls on multi-line edit controls with multi-line text
        //-------------------------------------------------------------------
        internal void Bug1113251( string bugID, string controls, string bugTitle )
        {
            string text = "";
            TextPatternRange range = null;
            TextPatternRange   targetRange   = null;
        ...
        //-------------------------------------------------------------------
        // Regress Bug 1040436: BVT Blocker: TextPatternRange--Expanding empty range via TextUnit.Line on text in a single-line Win32.Edit leaves an empty range
        //-------------------------------------------------------------------
        internal void Bug1040436( string bugID, string controls, string bugTitle )
        {
            int actualCount = 0;
            string text = "";
            TextPatternRange range = null;
        ...
        //-------------------------------------------------------------------
        // Regress Bug 1177689: Fail in casting obj to int[] at RuntimeId
        //-------------------------------------------------------------------
        internal void Bug1177689(string bugID, string controls, string bugTitle)
        {
            TS_RegressionTest(bugID, controls, bugTitle);
        ...
        // Regress Bug 
        //-------------------------------------------------------------------
        internal void Bug1744558(string bugID, string controls, string bugTitle)
        {
            bool                compare     = false;
            int                 actualCount = 0;
            TextPatternRange    range       = null;
