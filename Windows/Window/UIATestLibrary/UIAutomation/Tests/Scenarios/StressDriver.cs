                ...
                if (method != null)
                {
                    try
                    {
                        if (dumpStressBucketAttribute)
                            ...
                            UIVerifyLogger.LogComment("TBD"); //(StressBuckets)(method.GetCustomAttributes(false)[0]), true, null);
...
namespace UIAStressHelpers
{
    ...
    /// 
    /// TODO$ : What is the case if other groups want to add AE's so that
    /// we can test them also.  May need two copies in the collection, one
    /// that is the wrapped by UIAutomationTestTypes, so that we can then 
    /// reference this collection with RemoveObject with the wrapped 
    /// AutomationElement as an argument.
    /// </summary>
    /// -----------------------------------------------------------------------
