...
namespace Microsoft.Test.UIAutomation.Tests.Scenarios
{
    ...
    public class TopLevelEventsScenarioTests : ScenarioObject
    {
        ...
        private void TS_OpenRunDialog(out AutomationElement runDialog, CheckType checkType)
        {
            ...
            ATGTestInput.Input.SendKeyboardInput(new Key[] { Key.LWin, Key.R });
            ...
            for (;;)
            {
                if (DateTime.Now > maxTime)
                {
                    ThrowMe(checkType, "Could not find the run dialog after pressing WinKey+R");
                }
        ...
        /// <summary></summary>
        /// -------------------------------------------------------------------
        [TestCaseAttribute("ToolTipEvent.1",
            ...
            Description = new string[] {
                "Step: Move mouse away from start menu",
                "Step: Get the Start menu button",
                "Precondition: Verify that the start menu is visible",
                "Step: Add WindowOpenedEvent listener for application",
