...
namespace Bot.Activity.UserInputs
{
    ...
    public class UserInput_UseIdPassword : BaseNativeActivity
    {
        [RequiredArgument]
        ...
        protected override void Execute(NativeActivityContext context)
        {
            try
            {
                ...
                Bookmark bk = context.CreateBookmark("UseIdPassword", new BookmarkCallback(OnResumeBookmark));
                ThreadInvoker.Instance.RunByUiThread(() =>
                {
                    UserIdPassword useridpwd = new UserIdPassword(ref userInputEntity);
                    useridpwd.ShowDialog();
                    if (useridpwd.DialogResult == true)
                    {
                        dialogResult = true;
                        SelectHelper._wfApplication.ResumeBookmark("UseIdPassword", "");
                    }
                });
            }
            catch (Exception ex)
            {
                ...
                Log.Logger.LogData(ex.Message + " in activity UserInput_UseIdPassword", LogLevel.Error);
                if (!ContinueOnError)
                {
                    context.Abort();
                }
