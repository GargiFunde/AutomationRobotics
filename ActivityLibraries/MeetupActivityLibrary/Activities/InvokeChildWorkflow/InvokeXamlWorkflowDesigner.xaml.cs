namespace Bot.Activity.ActivityLibrary.Activities
{
    ...
    /// </summary>
    public partial class InvokeXamlWorkflowDesigner 
    {
        /// <summary>
        ...
        /// </summary>
        public InvokeXamlWorkflowDesigner()
        {
            InitializeComponent();
        ...
        //Open XAML Workflow
        private void OpenWorkflow(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                Btn_OpenWorkflow.CommandParameter = this.ModelItem.Properties["WorkflowPath"].ComputedValue.ToString();
                Btn_OpenWorkflow.Command = CustomCommands.CmdLoadInvokedWf;

            }
            catch(Exception ex)
            {
                Logger.Log.Logger.LogData("Exception While Opening XAML : "+ex.Message,Logger.LogLevel.Error);
            }
         }
        ...
        private void InitImportDynamicArgumentDialog()
        {
            ...
            {
                Dictionary<string, Argument> oldargumentDictionary = new Dictionary<string, Argument>(); //akf
                oldargumentDictionary = argumentDictionary;
                argumentDictionary = new Dictionary<string, Argument>();
                ...
                try
                {
                    ...
                    if (dynamicActivity != null)
                    {
                        ...
                        //to retain already existing values of arguments
                        if (oldargumentDictionary.Count != 0)
                        {
                            foreach (var oldkey in oldargumentDictionary.Keys)
                            { 
                                foreach (var newkey in argumentDictionary.Keys)
                                {
                                if (oldkey.Equals(newkey))
                                {
                                        argumentDictionary[newkey] = oldargumentDictionary[oldkey];
                                    break;
                                }
