...
public class CronExpression
{
    ...
    private void BuildCronExpression()
    {
        switch (ExpressionType)
        {
            ...
            case CronExpressionType.EveryDayAt:
                ...
                _cronString = string.Format("0 {0} {1} ? * * *", _startMinute, _startHour, _interval);
                break;
            ...
            case CronExpressionType.EverySpecificWeekDayAt:
                ...
                break;
            case CronExpressionType.EverySpecificDayMonthAt:
                _cronString = string.Format("0 {0} {1} ? 1/{2} {3} *", _startMinute, _startHour, (int)_interval, CronConverter.ToCronRepresentation(_days));
                break;
    ...
    protected CronExpression(DaysOfWeek days, int startHour, int startMinute, int interval, CronExpressionType expressionType)
    {
        _days = days;
        _startHour = startHour;
        _startMinute = startMinute;
    ...
    public static CronExpression EveryNDaysAt(int daysInterval, int hour, int minute)
    {
        ...
        //ashu
        DateTime date = DateTime.Now.Date.Add(new TimeSpan(hour, minute, 0));
        DateTime dateUTC= date.ToUniversalTime();
        int hourUTC = dateUTC.Hour;
        int minuteUTC = dateUTC.Minute;
        var ce = new CronExpression(daysInterval, hourUTC, minuteUTC, CronExpressionType.EveryNDaysAt);
        return ce;
    ...
    /// <returns>New CronExpression instance</returns>
    public static CronExpression EverySpecificDayMonthAt(int monthInverval, int hour, int minute, DaysOfWeek days)
    {
        //ashu
        DateTime date = DateTime.Now.Date.Add(new TimeSpan(hour, minute, 0));
        DateTime dateUTC = date.ToUniversalTime();
        int hourUTC = dateUTC.Hour;
        int minuteUTC = dateUTC.Minute;
        var ce = new CronExpression(days, hourUTC, minuteUTC, monthInverval, CronExpressionType.EverySpecificDayMonthAt);
        return ce;


        //var ce = new CronExpression(days, hour, minute, monthInverval, CronExpressionType.EverySpecificDayMonthAt);
        //return ce;
    }
    /// <summary>
    /// Create new CronExpression instance, which occurs on specified days at specified hours
    ...
    public static CronExpression EverySpecificWeekDayAt(int hour, int minute, DaysOfWeek days)
    {
        ...
        int hourUTC = dateUTC.Hour;
        int minuteUTC = dateUTC.Minute;
        var ce = new CronExpression(days, hourUTC, minuteUTC, CronExpressionType.EverySpecificWeekDayAt);
        return ce;
