...
public partial class Control_Tower_AddRole : System.Web.UI.Page
{
    ...
    protected void ImageButton1_Command(object sender, CommandEventArgs e)
    {
        ...
        string roleName = commandArgs[2].ToString().ToLower();
        int groupid = Convert.ToInt32(commandArgs[3]); 
        _HRoleid.Value = roleid.ToString();
        _HTenantID.Value = tenantid.ToString();
        _RoleName.Value = roleName.ToString();

        _HGroupid.Value = groupid.ToString();
        BindPopupData(roleid, groupid, tenantid, roleName);
    ...
    protected void BindPopupData(int roleid,int groupid, int tenantid, string roleName)
    {
        DataTable result = serviceInterface.GetRoleAccess(roleid, groupid, tenantid, roleName);
        if (result != null && result.Rows.Count > 0 && result.Rows[0][1] != DBNull.Value)
        {
            //check for the pages
            chkBotDashboardR.Checked = Convert.ToBoolean(result.Rows[0][1]);
    ...
    protected void BtnSaveRole(object sender, EventArgs e)
    {
        string StrRoleName = txtRole.Text.Trim();
        string StrGroupName = DrpGroupName.SelectedValue;

        if ((StrRoleName == "") || StrGroupName == "--Select--")
        {
        ...
        else
        {
            ...
            int groupid = (int)Session["GroupId"];
            int tenantid = (int)Session["TenantId"];
            int result = serviceInterface.AddRole(StrRoleName, groupid, tenantid, strcurrentuser, StrGroupName);
            if (result > 0)
            {
                serviceInterface.insertLog("Group Addedd Successfully", "Group Name " + StrRoleName + " has been Added successfully!", groupid, tenantid);
                ScriptManager.RegisterStartupScript(this, this.GetType(), "SweetAlert", " Swal.fire({position: 'top-end',type: 'success',title: '" + (String)GetGlobalResourceObject("content", "LitROLEADDEDSUCCESSFULLY") + "',text:'" + (String)GetGlobalResourceObject("content", "LitUserRole") + " " + StrRoleName + " " + (String)GetGlobalResourceObject("content", "Lithasbeenaddedsuccessfully") + "', showConfirmButton: false,  timer: 1500});", true);
            }
            else
            {
                serviceInterface.insertLog("Failed Adding Group", "Group Name " + StrRoleName + " has not been Added!", groupid, tenantid);
                ScriptManager.RegisterStartupScript(this, this.GetType(), "SweetAlert", " Swal.fire({position: 'top-end',type: 'error',title: '" + (String)GetGlobalResourceObject("content", "LitFAILEDROLEADDITION") + "',text:'" + (String)GetGlobalResourceObject("content", "LitUserRole") + " " + StrRoleName + " " + (String)GetGlobalResourceObject("content", "Lithasnotbeenadded") + "', showConfirmButton: false,  timer: 1500});", true);
            }
            getRoledata(tenantid);
        }
    }
    ...
    public void getRoledata(int tenantid)
    {
        ServiceInterface serviceInterface = new ServiceInterface();
        DataTable result = serviceInterface.getRoledata(tenantid);
    ...
    /*Method name : BtnCancelGroup
     ...
     Called from : UI  */
    protected void BtnCancelRole(object sender, EventArgs e)
    {
    ...
    protected void btnDelete_Click(object sender, CommandEventArgs e)
    {
        try
        {
            ...
            if (Session["TenantId"] != null || Session["GroupId"] != null)
            {
                ...
                string tenentid = commandArgs[2];
                string strcurrentuser = (string)Session["UserName"];

                lblId.Text = roleid;
                lblGroupName.Text = roleName;
                lblGroupIdSecondPopUp.Text = roleid;
                lblGroupNameSecondPopUp.Text = roleName;
                lblIdThirdPopUp.Text = roleid;
                lblGroupNameThirdPopUp.Text = roleName;
    ...
    protected void CommandBtnDisable_Click(object sender, EventArgs e)
    {
        ...
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "SweetAlert", " Swal.fire({position: 'top-end',type: 'error',title: '" + (String)GetGlobalResourceObject("content", "LitGROUPENABLEDISABLEFAILED") + "',text:'" + (String)GetGlobalResourceObject("content", "LitAGGroupName") + "" + strGroupName + " " + (String)GetGlobalResourceObject("content", "Lithasnotbeenenabledisablesuccessfully") + "', showConfirmButton: false,  timer: 1500});", true);
            System.Web.UI.ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "openModalDeleteSecondPopUp();", true);
        }
        ...
        try
        {
            ...
            if (Session["tenantid"] != null || Session["groupid"] != null)
            {
                ...
                if (result > 0)
                {
                    ...
                    System.Web.UI.ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "openModalHideDelete();", true);
                    getRoledata(tenantid);
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "SweetAlert", " Swal.fire({position: 'top-end',type: 'success',title: '" + (String)GetGlobalResourceObject("content", "LitGROUPNAMEDELETEDSUCCESSFULLY") + "',text:'" + (String)GetGlobalResourceObject("content", "LitAGGroupName") + " " + strGroupName + " " + (String)GetGlobalResourceObject("content", "Lithasbeendeletedsuccessfully") + "', showConfirmButton: false,  timer: 1500});", true);
                }
                else
                {
                    ...
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "SweetAlert", " Swal.fire({position: 'top-end',type: 'error',title: '" + (String)GetGlobalResourceObject("content", "LitGROUPNAMENOTDELETEDSUCCESSFULLY") + "',text:'" + (String)GetGlobalResourceObject("content", "LitAGGroupName") + " " + strGroupName + "" + (String)GetGlobalResourceObject("content", "LitGROUPNAMENOTDELETEDSUCCESSFULLY") + "', showConfirmButton: false,  timer: 1500});", true);
                    System.Web.UI.ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "openModalHideDelete();", true);
                }
            }
        }
    ...
    {
        int roleid = Convert.ToInt32(_HRoleid.Value);
        int tenantid = Convert.ToInt32(_HTenantID.Value);
        string roleName = _RoleName.Value;
        int result = 0;
