...
namespace Bot.Activity.WinDriverPlugin
{
    public class RecordedUiTask
    {
        ...
        public static string OutputXPath { get; set; }
        ...
        {
            try {
                //Variable declaration and initialization to default values
                string outxpath = string.Empty;
                string windowpath = string.Empty;
                string panePath = string.Empty;
                int endofindex = 0;
                int startindex = 0;

                this.UiTaskName = EnumUiTaskName.LeftDblClick;
                this._strDescription = null;
                OutputXPath = this._strPath;

                MainWindow.s_mainWin.rtbXPath.Document.Blocks.Clear();
                //MainWindow.s_mainWin.rtbXPath.Document.Blocks.Add(new Paragraph(new Run(text)));

                outxpath = OutputXPath;
                outxpath = outxpath.Replace("\\", "\"");
                windowpath = "/Window";
                panePath = "/Pane";

                //Deleting from first "/" to second "/";

                int index = outxpath.IndexOf('/', outxpath.IndexOf('/') + 1);
                outxpath = outxpath.Remove(0, index);
                outxpath = outxpath.Replace("\\", "\"");

                outxpath = "\"" + outxpath + "\"";


                //endofindex = outxpath.IndexOf(windowpath, StringComparison.CurrentCulture);
                //outxpath = outxpath.Remove(startindex, endofindex);
                //outxpath = outxpath.Remove(outxpath.Length - 1);
                //outxpath = "\"" + outxpath + "\"";

                MainWindow.s_mainWin.rtbXPath.AppendText(outxpath);
            }
            catch (Exception )
            {
