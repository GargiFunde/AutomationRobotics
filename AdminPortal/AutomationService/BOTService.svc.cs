...
namespace AutomationService
{
    ...
    public class BOTService : IBOTService
    {
        ...
        public BOTService()
        {
            ...
            //if (Connection == null || Connection.State == Microsoft.AspNet.SignalR.Client.ConnectionState.Disconnected) //Connecting to Scheduler Temp Directly
            //{
            //    connectAsync();
            //}
        ...
        #region GetCredentials For Activity
        public DataTable GetCredentials(string credentialName, int TenantId, int groupid)
        {
            ...
            using (SqlConnection conn = GetConnection())
            {
                try
                {
                    SqlCommand command = new SqlCommand("um_getCredentialsForDesigner", conn);
                    //SqlCommand command = new SqlCommand("um_getAssets", conn);
                    command.CommandType = CommandType.StoredProcedure;
                    var parameter = command.CreateParameter();
                    parameter.ParameterName = "credentialName";
                    parameter.DbType = System.Data.DbType.String;
                    parameter.Value = credentialName;
        ...
        //public bool AutomationRequestPriority(string iQueueName, string strRoutingKey, string iMessage, int iMsgPriority, string iGroupName, string iTenantName)
        //{
        //    string strExchangeTopic = ConfigurationManager.AppSettings["ExchangeTopicName"];
        //    string strRoutingLevel = ConfigurationManager.AppSettings["RoutingSupport"];
        ...
        //    //How Will you give me this ?
        //    DataTable dt = GetRQDetailsByName(iGroupName, iTenantName);
        //    if (dt.Rows.Count > 0)
        //    {
        //        var factory = new ConnectionFactory()
        //        {
        ...
        //            HostName = dt.Rows[0][0].ToString(),
        //            Port = (int)dt.Rows[0][1],
        //        };
        ...
        //                channel.QueueDeclare(iQueueName, true, false, false, Args);
        //                channel.QueueBind(iQueueName, strExchangeTopic, strRoutingKey);
        //                var props = channel.CreateBasicProperties();
        //                props.DeliveryMode = 2; //persistent
        //                props.Headers = new Dictionary<string, object>();
        //                props.Headers["source_message_id"] = "MsgReqNo_" + DateTime.Now.Ticks.ToString();
        //                props.Persistent = true;
        //                props.Priority = Convert.ToByte(iMsgPriority);
        //                var message = JsonConvert.SerializeObject(iMessage);
        //                //var message = XMLHelper.Serialize(iMessage);
        //                var body = Encoding.UTF8.GetBytes(message);
        //                channel.BasicPublish(exchange: strExchangeTopic,
        ...
        #region RabbitMQ Queing usinng Priority.
        //Added on 04-March-2020.
        //Description: Will add Items to RabbitMQ using Priority(Low,Medium,High) using Activity. GroupName and TenantName are the compulsary parameters required to Execute.
        //Activity Inherting: InsertQueueItems.
        public bool AutomationRequestPriority(string iQueueName, string strRoutingKey, Dictionary<string, object> iMessage1, int iMsgPriority, string iGroupName, string iTenantName)
        {
            string strExchangeTopic = ConfigurationManager.AppSettings["ExchangeTopicName"];
            string strRoutingLevel = ConfigurationManager.AppSettings["RoutingSupport"];
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                var factory = new ConnectionFactory()
                {
                    ...
                    //    CertPath = "C:\\result\\client_key.p12",
                ...
                try
                {
                    ...
                    using (var channel = connection.CreateModel())
                    {
                        ...
                        // var message = XMLHelper.Serialize(iMessage);//iska kya karu?

                        /*Commenting Because need to*/
                        //var message = JsonConvert.SerializeObject(iMessage);

                        //using The custom Data Type
                        QueueValue obj = new QueueValue();
                        obj.MessagePriority = iMsgPriority;
                        obj.Items = iMessage1;
        ...
        //Added on 04-March-2020.
        //Description: Will be used to get Queue Items according to priority.
        //Activity Inherting: GetQueueItems.
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                try
                {
                    ...
                    using (var channel = connection.CreateModel())
                    {
                        ...
                        channel.BasicConsume(iQueueName, false, consumer);
                        var ea = (BasicDeliverEventArgs)consumer.Queue.Dequeue();
                        var body = ea.Body;
                        var basicProperties = ea.BasicProperties;
                        var message = Encoding.UTF8.GetString(body);
                        channel.BasicAck(ea.DeliveryTag, false);
                        string priority = basicProperties.Priority.ToString();
                        //queueMessage = JsonConvert.DeserializeObject(message).ToString();
                        //queueMessage = queueMessage + "," + priority;
        ...
        private SqlConnection GetConnection()
        {
            ...
            //    connstring = ConfigurationManager.ConnectionStrings["ConnectionStringSql"].ConnectionString;
        ...
        #region GetAssets for Activity
        public DataTable GetAssetsActivity(string assetname, int groupid, int tenantid)
        {
            DataSet ds = new DataSet();
            DataTable dt = null;
            try
            {
                using (SqlConnection conn = GetConnection())
                {
                    SqlCommand command = new SqlCommand("um_getAssetsForDesigner", conn);
                    command.CommandType = CommandType.StoredProcedure;
                    var parameter = command.CreateParameter();
                    parameter.ParameterName = "assetName";
                    parameter.DbType = System.Data.DbType.String;
                    parameter.Value = assetname;
                    command.Parameters.Add(parameter);
        ...
        #region GetCredentials For Activity
        public DataTable GetCredentialsActivity(string credentialName, int TenantId, int groupid)
        {
            DataSet ds = new DataSet();
            DataTable dt = null;
        ...
        public string getKey()
        {
            ...
            using (SqlConnection conn = GetConnection())
            {
                try
                {
                    SqlCommand command = new SqlCommand("um_getKey", conn);
                    command.CommandType = CommandType.StoredProcedure;
                    ...
                    parameterreturn.DbType = System.Data.DbType.String;
                    parameterreturn.Size = 100;
                    ...
                    conn.Close();
                    result = Convert.ToString(parameterreturn.Value);
        ...
        public int DeleteCredential(int id, int groupid, int tenantid, string CredentialName, string user)
        {
            ...
            using (SqlConnection conn = GetConnection())
            {
                try
                {
                    SqlCommand command = new SqlCommand("um_deleteCredential", conn);
                    command.CommandType = CommandType.StoredProcedure;
                    ...
                    //command = AddAuditParameters(command, user);
        ...
        public int DeleteAsset(int id, int groupid, int tenantid, string AssetName, string user)
        {
            ...
            using (SqlConnection conn = GetConnection())
            {
                try
                {
                    SqlCommand command = new SqlCommand("um_deleteAsset", conn);
                    command.CommandType = CommandType.StoredProcedure;
        ...
        public DataTable getCredentials(int groupid, int tenantid)
        {
            ...
            try
            {
                using (SqlConnection conn = GetConnection())
                {
                    SqlCommand command = new SqlCommand("um_getCredentials", conn);
                    command.CommandType = CommandType.StoredProcedure;
                    AddGroupAndTenantParametes(ref command, groupid, tenantid);
                    SqlDataAdapter da = new SqlDataAdapter(command);
                    da.Fill(ds);
        ...
        public int AddAssets(string strAssetName, string strValue, int tenantid, int groupid, string strcreatedBy)
        {
            ...
            using (SqlConnection conn = GetConnection())
            {
                try
                {
                    SqlCommand command = new SqlCommand("um_createAsset", conn);
                    command.CommandType = CommandType.StoredProcedure;
                    var parameter = command.CreateParameter();
                    parameter.ParameterName = "assetname";
                    parameter.DbType = System.Data.DbType.String;
                    parameter.Value = strAssetName;
                    command.Parameters.Add(parameter);
                    ...
                    command = AddAuditParameters(command, strcreatedBy);
        ...
        public int AddCredentials(string strCredential, string strUserName, string Encrypted_Pwd, int tenantid, int groupid, string strcreatedBy)
        {
            ...
            using (SqlConnection conn = GetConnection())
            {
                try
                {
                    SqlCommand command = new SqlCommand("um_createCredential", conn);
                    command.CommandType = CommandType.StoredProcedure;
                    var parameter = command.CreateParameter();
                    parameter.ParameterName = "credentialname";
                    parameter.DbType = System.Data.DbType.String;
                    parameter.Value = strCredential;
                    command.Parameters.Add(parameter);
        ...
        //public int createKey(string key, string assetName, int groupid, int tenantid)
        //{
        //    int i = 0;
        ...
        //        try
        //        {
        //            SqlCommand command = new SqlCommand("um_createKey",conn);
        //            command.CommandType = CommandType.StoredProcedure;

        //            var parameter = command.CreateParameter();
        //            parameter.ParameterName = "assetkey";
        ...
        public int getRoleId(int TenantId, string rolename, int groupid)
        {
            ...
            using (SqlConnection conn = GetConnection())
            {
                try
                {
                    SqlCommand command = new SqlCommand("um_getRoleId", conn);
                    command.CommandType = CommandType.StoredProcedure;
...

        {
            ...
            using (SqlConnection conn = GetConnection())
            {
                try
                {
                    SqlCommand command = new SqlCommand("um_updateQueueToBotMapping", conn);
                    command.CommandType = CommandType.StoredProcedure;
...

        {
            ...
            using (SqlConnection conn = GetConnection())
            {
                try
                {
                    SqlCommand command = new SqlCommand("um_updateBotNameTouserBotMapping", conn);
                    command.CommandType = CommandType.StoredProcedure;
        ...
        public DataTable GetRoleAccess(int Roleid, int groupid, int tenantid, string roleName)
        {
            ...
            using (SqlConnection conn = GetConnection())
            {
                try
                {
                    SqlCommand command = new SqlCommand("um_getroleaccess", conn);
                    command.CommandType = CommandType.StoredProcedure;
        ...
        public int SaveRoleAccess(string rolename, int roleid, int tenantid, string role, bool bBotDashboardR, bool bBotDashboardC, bool bBotDashboardE, bool bBotDashboardD, bool bqueueR, bool bqueueC, bool bqueueE, bool bqueueD, bool bAddScheduleR, bool bAddScheduleC, bool bAddScheduleE, bool bAddScheduleD, bool bAddUserR, bool bAddUserC, bool bAddUserE, bool bAddUserD, bool bAddRobotR, bool bAddRobotC, bool bAddRobotE, bool bAddRobotD, bool bQueueManagementR, bool bQueueManagementC, bool bQueueManagementE, bool bQueueManagementD, bool bAssignQueueBotR, bool bAssignQueueBotC, bool bAssignQueueBotE, bool bAssignQueueBotD, bool bAssignBotUserR, bool bAssignBotUserC, bool bAssignBotUserE, bool bAssignBotUserD, bool bBotLogR, bool bBotLogC, bool bBotLogE, bool bBotLogD, bool bAuditTrailR, bool bAuditTrailC, bool bAuditTrailE, bool bAuditTrailD, bool bScheduleDetailsR, bool bScheduleDetailsC, bool bScheduleDetailsE, bool bScheduleDetailsD, bool bConfigurationR, bool bConfigurationC, bool bConfigurationE, bool bConfigurationD, bool bProcessManagementR, bool bProcessManagementC, bool bProcessManagementE, bool bProcessManagementD, bool bDetailLogR, bool bDetailLogC, bool bDetailLogE, bool bDetailLogD, bool bPromoteDemoteR, bool bPromoteDemoteC, bool bPromoteDemoteE, bool bPromoteDemoteD)
        {
            ...
            using (SqlConnection conn = GetConnection())
            {
                try
                {
                    SqlCommand command = new SqlCommand("um_saveroleaccessdetails", conn);
                    command.CommandType = CommandType.StoredProcedure;
        ...
        private void AddRoleAndTenantParametes(ref SqlCommand command, int roleid, int iTenantId)
        {
            var parameter = command.CreateParameter();
        ...
        public int AddRole(string RoleName, int groupid, int tenantid, string createdby, string groupname)
        {
            ...
            using (SqlConnection conn = GetConnection())
            {
                try
                {
                    SqlCommand command = new SqlCommand("um_createRole", conn);
                    command.CommandType = CommandType.StoredProcedure;
                    var parameter = command.CreateParameter();
                    parameter.ParameterName = "RoleName";
                    parameter.DbType = System.Data.DbType.String;
                    ...
                    AddRoleAndTenantParametes(ref command, groupid, tenantid);
        ...
        public DataTable getRoledata(int tenantid)
        {
            ...
            try
            {
                using (SqlConnection conn = GetConnection())
                {
                    SqlCommand command = new SqlCommand("um_getroles", conn);
        ...
        public DataTable getGrounNames(int tenantid)
        {
            ...
            try
            {
                using (SqlConnection conn = GetConnection())
                {
                    SqlCommand command = new SqlCommand("um_getgroupsnamesforrole", conn);
                    command.CommandType = CommandType.StoredProcedure;
        ...
        public DataTable getRoleType(string groupname, int groupid, int tenantid)
        {
            ...
            try
            {
                using (SqlConnection conn = GetConnection())
                {
                    SqlCommand command = new SqlCommand("um_getroleforuser", conn);
                    command.CommandType = CommandType.StoredProcedure;
        ...
        public DataTable GetProcessQMapping(int groupid, int tenantid)
        {
            ...
            using (SqlConnection conn = GetConnection())
            {
                SqlCommand command = new SqlCommand("um_getprocessqmapping", conn);
                command.CommandType = CommandType.StoredProcedure;
        ...
        public int AddProcessQueueMapping(string processName, string groupName, string tenantName)
        {
            ...
            using (SqlConnection conn = GetConnection())
            {
                try
                {
                    SqlCommand command = new SqlCommand("um_addprocessqueuemapping", conn);
                    command.CommandType = CommandType.StoredProcedure;
        ...
        public int deleteprocessQMapping(string processname, int GroupId, int TenanatId)
        {
            ...
            using (SqlConnection conn = GetConnection())
            {
                ...
                try
                {
                    SqlCommand command = new SqlCommand("um_deleteprocessQMapping", conn);
                    command.CommandType = CommandType.StoredProcedure;
                    var parameter = command.CreateParameter();
                    parameter = command.CreateParameter();
                    parameter.ParameterName = "@processname";
                    ...
                    AddGroupAndTenantParametes(ref command, GroupId, TenanatId);
        ...
        public DataTable getMicrosoftOCR()
        {
            try
            {
                ...
                using (SqlConnection conn = GetConnection())
                {
                    SqlCommand command = new SqlCommand("um_getMicrosoftOCR", conn);
                    command.CommandType = CommandType.StoredProcedure;
