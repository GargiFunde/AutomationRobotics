...
namespace Bot.Activity.PowerShell
{
    ...
    [Designer(typeof(ActivityDesignerForPowerShell_ExecuteCommandForAdmin))]
    public class PowerShell_ExecuteCommandForAdmin : BaseNativeActivity
    {     
        [RequiredArgument]
        ...
        protected override void Execute(NativeActivityContext context)
        {
            try
            {
                ...
                System.Collections.ObjectModel.Collection<PSObject> results = psEngine.ExecuteScript(pscommand, null, computername);
                Result.Set(context, results);
            }
            catch (Exception ex)
            {
                Log.Logger.LogData(ex.Message + " in activity PowerShell_ExecuteCommandForAdmin", LogLevel.Error);
                if (!ContinueOnError)
                {
                    context.Abort();
                }
