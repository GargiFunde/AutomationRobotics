// <copyright file=Receiver company=E2E BOTS>
...
namespace TCP.Server
{
    public class Receiver
    {
        ...
        public void Start()
        {
            ...
            System.IO.File.AppendAllText(@"C:\Piyush\DebugService.txt", "Reciever.cs=> ");
        ...
        private void Disconnect()
        {
            ...
            {
                if (Server.Receivers.Any(x => x.Email == OtherSideReceiver.Email))
                {
                    Receiver r = Server.Receivers.First(x => x.Email == OtherSideReceiver.Email);
                    Server.Receivers.Remove(r);
                }
        ...
        private void OnMessageReceived(MessageBase msg)
        {
            ...
            {
                TextMessageRequest tm =(TextMessageRequest) msg;
                if (tm.Message.Contains("heartbeat"))
                {
                    if (Server.LstDisconnected.Contains(this.Email))
                    {
                        Server.LstDisconnected.Remove(this.Email);
                    }
                }
                //Heartbeat requests from clients
            }
