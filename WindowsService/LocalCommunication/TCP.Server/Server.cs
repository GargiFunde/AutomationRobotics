// <copyright file=Server company=E2E BOTS>
...
namespace TCP.Server
{
    public class Server
    {
        ...
        public List<string> LstDisconnected { get; set; }
        ...
        public Server(int port)
        {
            ...
            LstDisconnected = new List<string>();
            Port = port;
            ClientValidating += server_ClientValidating;
        }
        void server_ClientValidating(EventArguments.ClientValidatingEventArgs args)
        {

            if (!Receivers.Exists(x => x.Email == args.Request.Email))
            {
            ...
            {
                Receiver r = Receivers.First(x => x.Email == args.Request.Email);
                if (r.Status == Shared.Enums.StatusEnum.Disconnected)
                {
                    Receivers.Remove(r);
                    args.Confirm();
                    return;
        ...
        public Receiver  GetBotListener(string ClientName)
        {         
            foreach (var receiver in Receivers)
            {
                if (receiver.Email == ClientName)
                {
                    System.IO.File.AppendAllText(@"C:\Work\SchedLogs.txt", System.DateTime.Now.ToString() + " : GetBotListener "+receiver.Email+" \n");
                    return receiver;
                }
            }
            return null;
        ...
        public void SendMessageToClient(string ClientName,string nsg)
        {
             foreach (var receiver in Receivers)
            {
                if (receiver.Email == ClientName)
                {
                    ...
                    request.Message = nsg;
                    System.IO.File.AppendAllText(@"C:\DebugService.txt", "Server.cs=>nsg: " + nsg + "Client Name: "+ ClientName + "\n");
        ...
        private void ConnectionHandler(IAsyncResult ar)
        {
            lock (Receivers)
            {
                System.IO.File.AppendAllText(@"C:\Piyush\DebugService.txt", "In server connection Handler" + "\t" );
