    ...
    public class AgentOperations //DefaultAgent
    {
        ...
        public bool StartRobot(string[] list)
        {
            try
            {
                ...
                servRef = new Logger.ServiceReference1.BOTServiceClient();
                string encryptionKey = servRef.getKey();
                System.IO.File.AppendAllText(@"C:\Piyush\DebugService.txt", "Start Robot Agent Operations EncryptionKey - " + encryptionKey + "  \n");
                EncryptionHelper encryptionHelper = new EncryptionHelper();
                pwd = encryptionHelper.Decrypt(pwd, encryptionKey).Trim();
                System.IO.File.AppendAllText(@"C:\Piyush\DebugService.txt", "Start Robot Agent Operations PWd - "+pwd+"  \n");
                //ProcessExtensions.StartProcessAsCurrentUser(applicationName); //Existing Mechanism for starting BackOfficeBot by creating a Windows Session
                string runnerpath = ConfigurationManager.AppSettings["Runner"];
                ProcessExtensions.StartProcessAsRobotUser(applicationName, botid, pwd, runnerpath);
                ...
                ////if (!botid.ToLower().Contains(Environment.MachineName.ToLower()))
                ////{
                ////   //Commented this : Onusing  below mechanism for bot id it is throwing exception use simple name
                ////   //botid = Environment.MachineName + "\\" + botid;
                ////}
                ...
                //process.StartInfo.FileName = applicationName.Trim();
                //process.StartInfo.Domain = Environment.UserDomainName;
                //process.StartInfo.UserName = botid.Trim();
            ...
            {
                Logger.Log.Logger.LogData("Exception whille starting the bot : " + ex.Message, LogLevel.Error);
            }
